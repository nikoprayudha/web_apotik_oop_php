...
if(isset($_REQUEST['submit'])){
    extract($_REQUEST);
    $obj->update_pemesanan($pemesananid,$supplierid,$profesiid,$tglpemesanan);
    }
if(isset($_REQUEST['idPemesanan'])){
    $idPemesanan=$_REQUEST['idPemesanan'];
    $result=$obj->mysqli->query("SELECT * FROM pemesanan where idPemesanan='$idPemesanan'");
...
<div class="graphs">
    <div class="bs-example4" data-example-id="simple-responsive-table">
        <h3>Forms Update Pemesanan</h3>
        <div class="tab-content">
            <div class="tab-pane active" id="horizontal-form">
                <!-- form action -->
                <form class="form-horizontal" action="update_pemesanan.php">
                    
                    <div class="form-group">
                        <label for="focusedinput" class="col-sm-2 control-label">Id Pemesanan</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control1" placeholder="Id Pemesanan" name="pemesananid" 
                            value="<?php echo $idPemesanan; ?>"  readonly/>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="selector1" class="col-sm-2 control-label">Id Supplier</label>
                       <div class="col-sm-8">
                        <select class="form-control1" name="supplierid" value="<?php echo $idSupplier; ?>" required>
                                <?php
                                $obj=new mysqli("localhost","root","","apotik");
                                $queryusers = "SELECT * FROM supplier ";
                                $obj = mysqli_query($obj, $queryusers);
                                while ( $d=mysqli_fetch_assoc($obj)) {
                                echo "<option>".$d['idSupplier']."</option>";
                                }
                                ?>
                        </select>
                        </div>
                     ...
                     <div class="form-group">
                        <label for="selector1" class="col-sm-2 control-label">Id Profesi Karyawan</label>
                       <div class="col-sm-8">
                        <select class="form-control1" name="profesiid" value="<?php echo $idProfesikrw; ?>" required>
                             
                                <?php
                                $obj=new mysqli("localhost","root","","apotik");
                                $queryusers = "SELECT * FROM profesikrw ";
                                $obj = mysqli_query($obj, $queryusers);
                                while ( $d=mysqli_fetch_assoc($obj)) {
                                echo "<option>".$d['idProfesikrw']."</option>";
                                }
                                ?>
                        </select>
                        </div>
                    ...
                    <div class="form-group">
                        <label for="focusedinput" class="col-sm-2 control-label">Tanggal Pemesanan</label>
                        <div class="col-sm-8">
                            <input type="date" class="form-control1" name="tglpemesanan" value="<?php echo $tglPemesanan; ?>" required>
                        </div>
                    </div>
